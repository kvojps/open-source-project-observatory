    ...
    def from_repository_response(
        ...
        return cls(
            ...
            issues_graphic=IssueGraphicResponse(
                open_issues=repository_data.get("openIssues", None).get(
                    "totalCount", None
                ),
                closed_issues=repository_data.get("closedIssues", None).get(
                    "totalCount", None
                ),
            ),
            prs_graphic=PullRequestGraphicResponse(
                open_pull_requests=repository_data.get("openPullRequests", None).get(
                    "totalCount", None
                ),
                closed_pull_requests=repository_data.get(
                    "closedPullRequests", None
                ).get("totalCount", None),
